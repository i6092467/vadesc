...
from datasets.hgg.hgg_data import generate_hgg_full

from utils.plotting import plot_overall_kaplan_meier, plot_group_kaplan_meier, plot_bigroup_kaplan_meier, \
    plotting_setup, plot_tsne_by_cluster, plot_tsne_by_survival
...
def run_experiment(args, configs, loss):
    ...
    for i in range(args.runs):
        ...
        if args.lrs:
            cp_callback = [tf.keras.callbacks.TensorBoard(log_dir='models/logs/' + ex_name),
        ...
        f.write(
            ...
            % (args.num_epochs, args.batch_size, configs['training']['latent_dim'], configs['training']['num_clusters'], configs['training']['monte_carlo'],
        ...
        if args.data == 'nsclc':
            ...
            plot_group_kaplan_meier(t=y_train[:, 0], d=y_train[:, 1], c=yy, dir='', experiment_name='lung1_' + str(args.seed))
        ...
        f.write("Test (w/o t)   |   Accuracy: %.3f, NMI: %.3f, ARI: %.3f. CI: %.3f, RAE (nc.): %.3f, RAE (c.): %.3f, CAL: %.3f.\n" % (
        acc, nmi, ari, ci, rae_nc, rae_c, cal))
        tf.keras.backend.set_value(model.use_t, np.array([1.0]))

        if args.data == 'lung1':
            np.savetxt(fname="preds_lung1_" + str(args.seed) + ".csv", X=np.stack((t_pred_med, y_test[:, 0], y_test[:, 1]),axis=1))
        elif args.data == 'nsclc':
            np.savetxt(fname="preds_nsclc_" + str(args.seed) + ".csv", X=np.stack((t_pred_med, y_test[:, 0], y_test[:, 1]),axis=1))
        elif args.data == 'basel':
    ...
    if args.runs > 1:
        ...
        f.write("Epochs= %d, batch_size= %d, latent_space= %d, mc samples= %d, weibull_shape= %d, learning_rate= %f, "
                "pretrain_e= %d, decay= %f, runs= %d, name= %s, "
                "seed= %d.\n "
                % (args.num_epochs, args.batch_size, configs['training']['latent_dim'],
                   configs['training']['weibull_shape'],
                   configs['training']['monte_carlo'], args.lr, args.epochs_pretrain, args.decay, args.runs, ex_name,
                   args.seed))
        if args.pretrain:
            f.write("Pretrain accuracy: %f , " % (pretrain_acc))
        f.write("Accuracy: %.3f std %.3f, NMI: %.3f std %.3f, ARI: %.3f std %.3f, CI: %.3f std %.3f. \n" % (
